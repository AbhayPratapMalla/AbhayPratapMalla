## Hi there I'm Abhay Pratap Malla 👋
🎓 International MSc student in Logistics & Supply Chain Management (LSCM)  
📊 Aspiring Supply Chain Analyst  
📍 Passionate about data-driven decision making in supply chain operations  
🚀 About Me

I am currently pursuing my MSc in Logistics & Supply Chain Management and building strong analytical skills to transition into a Supply Chain Analyst role.
I enjoy working with data, solving operational problems, and improving efficiency in supply chain systems using analytics and technology.
 🧠 Skills I'm Learning

### 📊 Data & Analytics
- Excel (Dashboarding, Pivot Tables, Data Analysis)
- SQL (Data extraction, queries, reporting)
- Power BI (Dashboards & visualization)
- Python (Data analysis & automation)

### ⚙️ Supply Chain & Tools
- SAP (ERP fundamentals)
- AWS (Cloud basics for data & analytics)

## 🎯 Career Goal
To become a **Supply Chain Analyst** specializing in:
- Data-driven supply chain optimization
- Demand forecasting
- Inventory and logistics analytics

## 📫 Connect with Me
- LinkedIn: www.linkedin.com/in/abhay-pratap-malla-295a1a303
- Email: abhaypratapmalla20@gmail.com

⭐ *Always learning, building, and improving one project at a time.*


Here are some ideas to get you started:
